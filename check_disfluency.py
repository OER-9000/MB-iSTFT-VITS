...
def check_disfluency_in_utterance(uudbroot, session_id, utterance_id):
    """
    Checks if any chunk in a specific utterance has the Disfluency="true" tag.
    """
    ...
    try:
        ...
        # This case should ideally not happen if the input is correct
        return False
    return False

def main():
    parser = argparse.ArgumentParser(
        description='Check for "Disfluency" tags in the combined validation set.',
    ...
    disfluency_count = 0
    disfluent_utterances = []

    print("2. Checking each utterance for disfluency tags in XML files...")
    for utt_id_str in utterance_ids:
        # utt_id_str format is C001_L_008
        parts = utt_id_str.split('_')
        session_id = parts[0]
        utterance_id = parts[2]

        if check_disfluency_in_utterance(args.uudbroot, session_id, utterance_id):
            disfluency_count += 1
            disfluent_utterances.append(utt_id_str)

    print("\n--- Results ---")
    print(f"Total utterances checked: {len(utterance_ids)}")
    print(f"Number of utterances with Disfluency tag: {disfluency_count}")
    
    if disfluency_count > 0:
        print("\nUtterances with Disfluency tag:")
        for utt in disfluent_utterances:
            print(utt)
