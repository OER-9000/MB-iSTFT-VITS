...
class TTS_VITS(RemdisModule):
    ...
    def synthesis_loop(self):
        ...
        while self._is_running:
            ...
            try:
                in_msg = self.input_iu_buffer.get(block=True, timeout=1) # Add timeout
                try:
                    ...
                    if output_text != '':
                        ...
                        with torch.no_grad():
                            ...
                            print("[DEBUG] Synthesis - Step 4: Inferring from model")
                            ...
                            print(f"[DATA_INSPECT] Raw model output stats: Min={x.min():.4f}, Max={x.max():.4f}, Mean={x.mean():.4f}")
                            print("[DEBUG] Synthesis - Step 5: Inference complete")
                        
                        print("[DEBUG] Synthesis - Step 6: Resampling audio")
                        # Resample
                        x = librosa.resample(x.astype(numpy.float32), orig_sr=sr, target_sr=self.rate)
                        print("[DEBUG] Synthesis - Step 7: Resampling complete")

                        # Auto-normalize audio amplitude
                        enable_normalization = self.config['TTS_VITS'].get('auto_normalize', True)
                        if enable_normalization and x.any(): # Check if the array is not all zeros
                            max_abs_val = numpy.abs(x).max()
                            if max_abs_val > 1e-6: # Avoid division by zero or very small numbers
                                print(f"[DEBUG] Synthesis - Step 7a: Normalizing audio from max abs value: {max_abs_val:.4f}")
                                x = x / max_abs_val
                                print(f"[DATA_INSPECT] Normalized audio stats: Min={x.min():.4f}, Max={x.max():.4f}, Mean={x.mean():.4f}")
                            else:
                                print("[DEBUG] Synthesis - Step 7a: Skipping normalization due to very small amplitude.")
                        else:
                            print("[DEBUG] Synthesis - Step 7a: Skipping normalization (disabled or empty audio).")
                        
                        # Chunk and send
                        print("[DEBUG] Synthesis - Step 8: Chunking and sending")
                        t = 0
                        while t <= len(x):
                            chunk = x[t:t+self.chunk_size]
                            chunk_b64 = base64.b64encode((chunk * 32767).astype(numpy.int16).tobytes()).decode('utf-8')
                            snd_iu = self.createIU(chunk_b64, 'tts', update_type)
                            snd_iu['data_type'] = 'audio'
                            self.output_iu_buffer.put(snd_iu)
                            t += self.chunk_size
                        print("[DEBUG] Synthesis - Step 9: Chunking and sending complete")
                    
                    # Send commit message after processing all chunks
